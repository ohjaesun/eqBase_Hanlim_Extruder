...
using static EQ.Core.Sequence.seq14Step;
namespace EQ.Core.Sequence
{
    public enum seq14_Sig // 이름 포맷 꼭 지켜야 함 (이름 + _Sig)  => XXX_Sig
    {
        TrayReady,      // 트레이 준비 완료 신호
        WorkComplete    // 작업 완료 신호
    }
    public enum seq14Step
    {
        Start,
        Step2,
        Step3,
    ...
    public class Seq14 : AbstractSeqBase<seq14Step>
    {

        public Seq14(SEQ seqManager, ACT actManager) : base(seqManager, actManager)
        {
            InitSignals<seq14_Sig>();
        }

        public override async Task doSequence()
        {
            // 'Step' 대신 'StepEnum'을 사용하면 switch가 더 명 αξ해집니다.
            switch (Step)
            {
                case Start:
                    {
